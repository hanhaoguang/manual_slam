/*
 * @Author: hhg
 * @Date: 2022-09-09 16:34:45
 * @LastEditors: hhg
 * @LastEditTime: 2022-09-11 16:46:50
 * @FilePath: /slam_ws/src/manual_slam/src/front_end/front_end.cpp
 * @Description: 
 * 
...
FrontEnd::FrontEnd()
    :local_map_ptr_(new CloudData::CLOUD()),
     ...
     filtered_cloud_ptr_(new CloudData::CLOUD()),
     filtered_local_map_ptr_(new CloudData::CLOUD()){
...
bool FrontEnd::Update(const CloudData& cloud_data, Eigen::Matrix4f& cloud_pose) {
    ...
    LOG(INFO) << cloud_data.cloud_ptr->size();
    LOG(INFO) << current_frame_.cloud_data.cloud_ptr->size();

    // CloudData::CLOUD_PTR filtered_cloud_ptr(new CloudData::CLOUD());
    frame_filter_ptr_->Filter(current_frame_.cloud_data.cloud_ptr, filtered_cloud_ptr_);
    LOG(INFO) << filtered_cloud_ptr_->size();

    static Eigen::Matrix4f step_pose = Eigen::Matrix4f::Identity();// 帧间相对位姿
    static Eigen::Matrix4f last_pose = init_pose_;// 上一帧位姿
    static Eigen::Matrix4f predict_pose = init_pose_;// 配准提供的位姿初值
    static Eigen::Matrix4f last_key_frame_pose = init_pose_;// 上一关键帧位姿
    // LOG(INFO) << init_pose_;
    ...
    // 不是第一帧，就正常匹配
    registration_ptr_->ScanMatch(filtered_cloud_ptr_, predict_pose, result_cloud_ptr_, current_frame_.pose);
    cloud_pose = current_frame_.pose;
    LOG(INFO) << cloud_pose;
...
bool FrontEnd::UpdateWithNewFrame(const Frame& new_key_frame) {
    ...
    // 由于用的是共享指针，所以直接复制只是复制了一个指针而已 Frame-CloudData-成员变量CLOUD_PTR
    // 此时无论你放多少个关键帧在容器里，这些关键帧点云指针都是指向的同一个点云，因为new_key_frame是成员变量，这里不reset的话，每次成员变量reset应该也行
    // 
    key_frame.cloud_data.cloud_ptr.reset(new CloudData::CLOUD(*new_key_frame.cloud_data.cloud_ptr));
    CloudData::CLOUD_PTR transformed_cloud_ptr(new CloudData::CLOUD());
    LOG(INFO) << "888888888888888888888888";
    
    // 更新局部地图
    local_map_frames_.push_back(key_frame);
    while (local_map_frames_.size() > static_cast<size_t>(local_frame_num_)) {
        local_map_frames_.pop_front(); // 应该是自动释放内存了
    }
    LOG(INFO) << "99999999999999999999999999";
    ...
    LOG(INFO) << "1010101001001";

    // 更新ndt匹配的目标点云
    // 关键帧数量还比较少的时候不滤波，因为点云本来就不多，太稀疏影响匹配效果
    LOG(INFO) << local_map_ptr_->size();
    ...
    LOG(INFO) << "1212121212121212";

    // 保存所有关键帧信息在容器里
    // 存储之前，点云要先释放，因为已经存到了硬盘里，不释放也达不到节省内存的目的
    // 因为点云存在硬盘所以点云释放，但是pose要存
    key_frame.cloud_data.cloud_ptr.reset(new CloudData::CLOUD());
    global_map_frames_.push_back(key_frame);
    LOG(INFO) << "13131313131313";
    std::cout <<"fdsfdsfdsfdsfds"<<std::endl;
